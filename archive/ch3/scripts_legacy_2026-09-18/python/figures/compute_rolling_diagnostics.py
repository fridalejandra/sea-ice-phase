...
ASSUMPTION TO CHECK: "SIE anomaly" isn't a column anywhere I can see, so
it's built here as sie_annual minus its 1979-2023 mean, UNDETRENDED --
matching your own §2.3 convention that Spearman is reported on the
undetrended series where a result depends on the choice. If what you
actually plotted before was something else (e.g. departure from the
invariant-cycle fit used in Fig. 4's attribution, which would need the R
output rather than annual_params.csv), tell me and I'll switch the source.

This is a different question from Fig. 6: Fig. 6 asks whether phase and
