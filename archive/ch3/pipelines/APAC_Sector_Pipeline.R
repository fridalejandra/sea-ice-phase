...
fit_sector <- function(raw_data, sector_col) {
  ...
  annual_fitted <- sie %>%
    ...
    summarise(
      ...
      # --- AMPLITUDE: peak-to-trough of the amplitude-adjusted fitted curve ---
      # fitted_amp is already back-transformed to SIE units (million km²).
      # Taking max - min per year gives the annual amplitude on the same scale
      # as amplitude_raw_yr, making them directly comparable.
      # NOTE: do NOT subtract fitted_invariant here — that gave the range of
      # the *difference* between two curves (scaling-factor space residual),
      # not the actual annual amplitude magnitude.
...
message("\n All component columns saved in daily_fitted.csv")
message("\n All component columns saved in daily_fitted.csv")