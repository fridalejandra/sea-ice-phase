...
DATE_START <- as.Date("1978-10-26")   # H&R's record start
...
fit_sector <- function(raw_data, sector_col) {
  ...
  if (have_rugarch) {
    ...
    gf <- tryCatch(ugarchfit(spec, sie$residual_apac[ok], solver = "nlminb"),
                   error = function(e) NULL)
    if (is.null(gf)) gf <- tryCatch(
      ugarchfit(spec, sie$residual_apac[ok], solver = "solnp"),
      error = function(e) NULL)
    if (!is.null(gf) && gf@fit$convergence == 0) {
      sie$volatility[ok]    <- as.numeric(sigma(gf))
      sie$est_anomaly_g[ok] <- as.numeric(fitted(gf))
      message("  GARCH ok (nlminb/solnp)")
    } else message("  GARCH failed/nonconverged; est_anomaly -> 11-day mean")
  }
