...
def plot_crossing_freq() -> None:
    ...
    # --- map panels ---
    map_panels = [
        ("(a) FS — climatological mean", clim_fs_s, (3, 2, 1),  cmap_clim,  norm_clim),
        ("(b) MS — climatological mean", clim_ms_s, (3, 2, 2),  cmap_clim,  norm_clim),
        ("(c) FS — post minus pre 2016", diff_fs_s, (3, 2, 3),  cmap_diff,  norm_diff),
        ("(d) MS — post minus pre 2016", diff_ms_s, (3, 2, 4),  cmap_diff,  norm_diff),
    ]
    ...
    cax1 = fig.add_axes([0.08, 0.635, 0.38, 0.015])
    ...
    cax2 = fig.add_axes([0.08, 0.355, 0.38, 0.015])
    ...
    for idx, (phase, sec_data, subplot_pos, panel_label) in enumerate([
        ("FS", sec_fs, (3, 2, 5),  "(e) FS — sector means"),
        ("MS", sec_ms, (3, 2, 6), "(f) MS — sector means"),
    ]):
    ...
    fig.subplots_adjust(hspace=0.35, wspace=0.05, top=0.95, bottom=0.05)
