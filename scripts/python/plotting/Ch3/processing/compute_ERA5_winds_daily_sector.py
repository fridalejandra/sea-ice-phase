...
masks = {"circumpolar": sector_mask.notnull()}
for code, name in SECTORS.items():
    masks[name] = sector_mask == code


def sector_means(field):
    """field on the SIE grid -> dict of area means per mask."""
    out = {}
    for name, m in masks.items():
        out[name] = float(field.where(m).weighted(weights).mean(dim=("y", "x")))
    return out
...
for year in tqdm(range(START_YEAR, END_YEAR + 1), desc="Years"):
    ...
    if os.path.exists(ypath):
        try:
            n_done = len(pd.read_csv(ypath))
            if n_done >= len(files):
                continue          # this year is complete
        except Exception:
            pass
