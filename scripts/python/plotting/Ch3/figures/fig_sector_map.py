...
Two-panel figure:
  Top    — circumpolar polar stereographic projection
           showing all 5 sectors filled with transparency
  Bottom — Southern Ocean rectangular view (Mercator-like)
           showing sectors in context with ocean basins

Sector longitude boundaries (advance season, Raphael & Hobbs 2014):
    Weddell         : -65° to -25°
    King Haakon     : -25° to  70°
    East Antarctica :  70° to 165°
    Ross            : 165° to -110°  (crosses dateline)
    ABS             : -110° to -65°

Latitude bounds: -90° to -40°
"""
...
# ── Sector definitions ────────────────────────────────────────────────────────
# Longitude boundaries (advance season, Raphael & Hobbs 2014)
SECTORS = {
    ...
    "Ross"           : {"lon_min": 165.0,  "lon_max": 250.0,  "color": "#4CAF50"},  # 250 = -110
    "ABS"            : {"lon_min": 250.0,  "lon_max": 295.0,  "color": "#F44336"},  # 295 = -65
}
...
def sector_polygon(lon_min, lon_max, lat_min=-90, lat_max=-40, n=100):
    """
    Generate a sector polygon as arrays of lons and lats.
    Handles the dateline crossing by working in 0-360 space.
    """
    # Convert to 0-360
    if lon_min < 0:
        lon_min += 360
...
# Gridlines
gl = ax_polar.gridlines(draw_labels=False, linewidth=0.4,
                         color="white", alpha=0.6, zorder=4)

# Sector labels on polar plot
label_lons = {
    "Weddell"        : -45,
    "King Haakon"    :  22,
    "East Antarctica": 117,
...
for name, lon in label_lons.items():
    ...
    ax_polar.text(lon, label_lats, name,
                  transform=ccrs.PlateCarree(),
                  ha="center", va="center",
...
# ── Bottom panel: Southern Ocean rectangular view ─────────────────────────────
...
# ── Shared legend ─────────────────────────────────────────────────────────────
handles = [
    mpatches.Patch(facecolor=props["color"], alpha=0.8,
                   edgecolor=props["color"], label=name)
    for name, props in SECTORS.items()
]
