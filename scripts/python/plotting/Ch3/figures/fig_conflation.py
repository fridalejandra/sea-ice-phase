...
2 rows × 3 columns. Full page width (7.5").
...
PEAK_WINDOW      = 30
LETTERS          = [["(a)", "(b)", "(c)"], ["(d)", "(e)", "(f)"]]
COL_TITLES       = ["Traditional SIE anomaly",
...
def draw_apac_decomposition(ax, data_yr, clim_peak, color, letter):
    ...
    # Place annotation away from the peak to avoid overlap
    ax_ymin, ax_ymax = clim.min(), clim.max()
    text_y = fitted_peak_val - (ax_ymax - ax_ymin) * 0.18
    text_x = fitted_peak_doy + 30 if phase_shift < 0 else fitted_peak_doy - 80
    ax.annotate(
        f"{phase_shift:+d} d",
