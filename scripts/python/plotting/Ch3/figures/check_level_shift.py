...
Usage: python check_level_shift.py [SECTOR] [INDEX] [SEASONS]   (default SIE_Weddell Nino34 annual,ADV,RET)
  e.g. python check_level_shift.py SIE_Weddell SAM JJA,annual
"""
import os
import sys
import numpy as np
...
SEASONS = sys.argv[3].split(",") if len(sys.argv) > 3 else ["annual", "ADV", "RET"]
...
for s in SEASONS:
    if f"{INDEX}_{s}" not in idx.columns:
        print(f"{INDEX}_{s} not in index file; columns start {list(idx.columns)[:12]}"); continue
    name = f"{INDEX}_{s}"
    line = f"{name:16s}"
    for c in cols:
