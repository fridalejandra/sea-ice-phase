"""
fig_wind_strengthening.py
Wind stress magnitude difference (color) + vectors, DJF and MAM.
"""
import os
import numpy as np
import xarray as xr
...
OUT = "fig_wind_strengthening.png"
...
def main():
    ...
    for season, months in SEASONS.items():
        ...
        all_diffs.append(mag_post - mag_pre)
    ...
    for ax, (season, months) in zip(axes, SEASONS.items()):
        ...
        im = ax.pcolormesh(x, y, mag_diff, transform=EASE_CRS, cmap="RdBu_r",
        ...
        q = ax.quiver(lon_sub, lat_sub, u_sub, v_sub, transform=PLATE,
                     ...
                     scale=QUIVER_SCALE, width=0.0032,
    ...
    fig.colorbar(im, ax=axes, orientation="vertical", fraction=0.03, pad=0.02,
                 label="\u0394 wind stress magnitude (Pa, post \u2212 pre)")
