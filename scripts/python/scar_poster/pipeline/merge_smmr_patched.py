# merge_smmr.py  (PATCHED: explicit float32 + _FillValue encoding on every write)
...
def build_encoding(ds):
    """Force every floating-point variable to float32 with an explicit
    _FillValue, so NaNs (missing/flagged data) are never silently coerced
    to 0 or garbage integers on write."""
    encoding = {}
    for var in ds.data_vars:
        if np.issubdtype(ds[var].dtype, np.floating):
            encoding[var] = {"dtype": "float32", "_FillValue": FILL_VALUE}
    return encoding


# step 1: merge each year
# NOTE: removed the "skip if already exists" shortcut - always rebuild the
# yearly cache fresh so newly-downloaded granules and the encoding fix are
# guaranteed to be picked up. If you want the cache-skip behavior back for
# speed on a truly unchanged year, re-add the os.path.exists check, but be
# aware it will also skip picking up this encoding fix for that year.
yearly_files = []
for year in tqdm(years, desc="Merging by year"):
    ...
    try:
        ...
        ds.to_netcdf(out_year, encoding=encoding)
        ds.close()
...
encoding = build_encoding(ds_all)
