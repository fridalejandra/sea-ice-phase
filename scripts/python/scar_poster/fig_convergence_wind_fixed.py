"""
fig_convergence_wind_fixed.py
FIXED: masks cells with insufficient ice-day coverage within the season
before computing pre/post means (prevents a transient month like Dec
from dominating a "DJF" label).
"""
import os
import numpy as np
import xarray as xr
...
OUT = "fig_convergence_wind_fixed.png"
...
def seasonal_prepost_masked(ds, var, months, negate=False, min_valid_frac=MIN_VALID_FRAC):
    da = ds[var]
    if negate:
        da = -da
    tn = _tname(da)
    ...
    valid_frac_pre = np.isfinite(pre_data).mean(axis=0)
    valid_frac_post = np.isfinite(post_data).mean(axis=0)

    keep = (valid_frac_pre >= min_valid_frac) & (valid_frac_post >= min_valid_frac)
    ...
    print(f"    kept {keep.sum()}/{keep.size} cells "
          f"({100*keep.sum()/keep.size:.1f}%) at min_valid_frac={min_valid_frac}")

    return pre_mean, post_mean


def seasonal_prepost_wind(ds, var, months):
    da = ds[var]
    tn = _tname(da)
    yrs = da[tn].dt.year
    da = da.sel({tn: ~yrs.isin(EXCLUDE_YEARS)})
...
def main():
    ...
    for season, months in SEASONS.items():
        print(f"{season}:")
        tx_pre, tx_post = seasonal_prepost_wind(wind_ds, "tau_x", months)
        ty_pre, ty_post = seasonal_prepost_wind(wind_ds, "tau_y", months)
        ...
        conv_pre, conv_post = seasonal_prepost_masked(conv_ds, CONV_VAR, months, negate=True)
    ...
    fig.suptitle(f"Westerlies strengthened (top) \u2014 convergence, masked to cells with"
                 f"\n\u2265{int(MIN_VALID_FRAC*100)}% ice-day coverage in both periods (bottom)",
                 fontsize=14, y=0.99)
