#!/usr/bin/env python3
"""
Compute monthly and seasonal climatological sea-ice edge "mean location"
and a distance-to-edge field using only SIC.

Bootstrap NSIDC 25 km grid:
- Distance is computed in grid-cells and converted to km by multiplying by `--grid-km`
  (default 25). This is intended for *stratification/diagnostics*, not precise geodesy.

Outputs (NetCDF):
- ice_edge_prob_*.nc : probability of SIC >= threshold (default 0.15)
- ice_mask_clim_*.nc : climatological ice region (prob >= prob_cut)
- ice_edge_mask_*.nc : edge pixels (ice adjacent to water)
- dist_to_edge_km_*.nc : distance to nearest edge pixel, inside ice only
...
# Diagnostic windows (Southern Hemisphere oriented; adjust later if needed)
SEASONS_CUSTOM = {
    "ADV_MAR_JUN": [3, 4, 5, 6],
    "RET_OCT_JAN": [10, 11, 12, 1],
}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Also compute DJF/MAM/JJA/SON in addition to custom windows.",
    )
...
def write_products(
    ...
    # Save individual products (small, explicit files)
...
def main() -> None:
    ...
    # Infer dims (not strictly needed for output, but good for sanity)
    _y_dim, _x_dim = infer_xy(sic, args.x, args.y, args.time)

    outroot = Path(args.outdir)

    # MONTHLY
    p_month = ice_probability_monthly(sic, args.time, args.threshold)
    for m in p_month["month"].values.tolist():
        p = p_month.sel(month=m).drop_vars("month")
        write_products(
            outroot / "monthly",
            f"month{int(m):02d}",
            p,
    ...
    # STANDARD SEASONS (optional)
    if args.use_standard_seasons:
        for name, months in SEASONS_STD.items():
            p = ice_probability_season(sic, args.time, args.threshold, months)
            write_products(
                outroot / "seasonal",
                name,
    ...
    print("Done. Wrote monthly and seasonal edge/distance products to:", outroot)
